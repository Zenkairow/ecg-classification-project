...
# Define Rhythm Classes Mapping (Local to this Specialist)
# We map the string labels to 0..N indices
...
class RhythmDataset(Dataset):
    def __init__(self, csv_file, root_dir, seq_len=5000):
        ...
        for idx, row in self.data_frame.iterrows():
             ...
             # Fallback if label is not in our list (should not happen if data prep is correct)
             if label_str not in CLASS_TO_IDX:
                 # Check if diagnostic_superclass helps
             ...
             if label_str in CLASS_TO_IDX:
                 ...
                 # If truly unknown, map to -1 (will filter or crash later if not handled)
                 # Ideally Stage 3 data only contains valid Rhythm classes
                 self.labels.append(0) # Default to 0? Or skip?
    ...
    def __getitem__(self, idx):
        ...
        if pd.isna(fname) or fname == 'nan' or fname == '':
             fname = str(row.get('filename_hr', '')) # Should be fixed by prep script
        
        file_path = os.path.join(self.root_dir, fname)
        
        # Robust loading
        if not os.path.exists(file_path):
             # Try simple fallback if prep script missed something
        ...
        label = CLASS_TO_IDX.get(label_str, 0) # Default to first class if mismatch
        
        return torch.tensor(signal, dtype=torch.float32), torch.tensor(label, dtype=torch.long)

def train_rhythm_specialist():
    print("--- Starting Stage 3: Rhythm Specialist ---")
    print(f"Classes ({len(RHYTHM_CLASSES)}): {RHYTHM_CLASSES}")
    
    # 1. Data
    dataset = RhythmDataset(csv_file=STAGE3_RHYTHM_DATA_PATH, root_dir=DATA_DIR)
    ...
    # Sampler for Intra-Class Imbalance
    print("Calculating Sampler Weights...")
    train_indices = train_subset.indices
    train_labels = dataset.labels[train_indices]
    
    class_counts = np.bincount(train_labels, minlength=len(RHYTHM_CLASSES))
