...
def train_model(class_weights):
    """
    Main function to orchestrate the training of the ResNet model on 500Hz data.
    """
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Load the full dataset using the updated 500Hz config
    ...
    print("Starting training with ResNet (500Hz) and dampened weighted loss...")
    ...
    if not os.path.exists('models'):
        os.makedirs('models')
    # *** KEY CHANGE: Save this as Version 5 of our model ***
    torch.save(model.state_dict(), 'models/ecg_model_v5_500hz.pth')
    print("Model saved to models/ecg_model_v5_500hz.pth")
...
if __name__ == '__main__':
    ...
    weights[3] = torch.sqrt(weights[3]) # Dampen CD
    weights[4] = torch.sqrt(weights[4]) # Dampen HYP

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    weights = weights.to(device)
    
    print(f"Using dampened class weights: {weights}")
